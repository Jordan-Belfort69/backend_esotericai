...
from app.services.rituals_service import (
    get_daily_tip_settings,
    upsert_daily_tip_settings,
)
...
async def get_settings(user_id: CurrentUserDep):
    settings = await get_daily_tip_settings(user_id)
    return DailyTipSettingsOut(**settings)


@router.post("/daily-tip-settings", response_model=DailyTipSettingsOut)
async def save_settings(payload: DailyTipSettingsIn, user_id: CurrentUserDep):
    settings = await upsert_daily_tip_settings(
        user_id=user_id,
...
@router.post(
    "/daily-tip-settings-test",
    response_model=DailyTipSettingsOut,
    include_in_schema=False,
)
async def save_settings_test(payload: DailyTipSettingsTestIn):
    """
